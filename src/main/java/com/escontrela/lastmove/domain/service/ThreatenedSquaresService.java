...
public final class ThreatenedSquaresService {
  ...
  /** Squares occupied by {@code attacker}'s opponent that are attacked by {@code attacker}. */
  public Set<Square> attackedPieces(PositionSnapshot position, PieceColor attacker) {
    Set<Square> attacked=attackedBy(position,attacker); Set<Square> result=new HashSet<>(); position.pieces().stream().filter(p->p.color()==attacker.opposite()&&attacked.contains(p.square())).forEach(p->result.add(p.square())); return Set.copyOf(result);
  }
