...
public final class PlayerSelectorModal extends StackPane {
  ...
  public PlayerSelectorModal() {
    ...
    card.getChildren().addAll(header, message, options, actions); card.getStyleClass().addAll("message-box-card", "player-selector-modal-card"); card.setPadding(new Insets(18));
  ...
  public void show(List<PlayerSummary> players, Long selectedPlayerId, String titleText, String messageText) {
    ...
    options.getChildren().setAll(players.stream().map(player -> option(player, selectedPlayerId)).toList());
  ...
  private Button option(PlayerSummary player, Long selectedPlayerId) {
    ...
    Label detail = new Label(player.systemPlayer() ? "Lichess bot" : "Local player"); detail.getStyleClass().add("player-selector-detail");
