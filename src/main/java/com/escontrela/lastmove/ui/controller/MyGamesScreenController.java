    ...
    games.findSaved(summary.gameId()).ifPresentOrElse(saved -> {
      ...
      computerViewer.showLichessViewer(game.lichessGameId(), summary.gameId(),
          new com.escontrela.lastmove.ui.component.game.LiveGameViewerState(com.escontrela.lastmove.ui.component.game.LiveGameViewerSource.LICHESS,
              ...
              record.moves().stream().map(com.escontrela.lastmove.domain.game.RecordedPly::ply).toList(), saved.game().currentClock().whiteRemaining(), saved.game().currentClock().blackRemaining(), record.timeControl().flatMap(com.escontrela.lastmove.domain.game.TimeControl::initialTime), record.result().isPresent(), record.result(), record.terminationReason(), Optional.of("Following Lichess game live")));
