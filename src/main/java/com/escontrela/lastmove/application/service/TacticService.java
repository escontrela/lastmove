...
public final class TacticService {
  ...
  public TacticService(
    ...
    this.tacticRepository = Objects.requireNonNull(tacticRepository, "tacticRepository must not be null");
    ...
    this.exerciseFactory = Objects.requireNonNull(exerciseFactory, "exerciseFactory must not be null");
  ...
  public List<TacticSuiteSummary> listSuites(PlayerId ownerId) {
    assertAvailable();
    return tacticRepository.findAllByOwner(Objects.requireNonNull(ownerId, "ownerId must not be null"))
  ...
  public TacticSuiteDetails suiteDetails(PlayerId ownerId, TacticSuiteId suiteId) {
    ...
    return new TacticSuiteDetails(summary(suite), suite.exercises().stream().map(this::summary).toList());
  }
  ...
  public TacticExerciseSummary createExerciseFromFen(CreateTacticExerciseFromFenCommand command) {
    ...
    TacticExercise exercise =
        exerciseFactory.empty(required.title(), gameFactory.createAnalysisGame(required.fen()).currentPosition());
  ...
  public TacticExerciseSummary copyAnalysisSessionTactic(CopyAnalysisSessionTacticCommand command) {
    ...
    var session =
        analysisSessionRepository
            ...
            .orElseThrow(
                () -> new NoSuchElementException("Unknown analysis session " + required.sessionId().value()));
  ...
  public TacticExerciseSummary renameExercise(RenameTacticExerciseCommand command) {
    assertAvailable();
    RenameTacticExerciseCommand required = Objects.requireNonNull(command, "command must not be null");
    TacticSuite suite = ownedSuite(required.ownerId(), required.suiteId());
  ...
  public boolean moveExercise(MoveTacticExerciseCommand command) {
    assertAvailable();
    MoveTacticExerciseCommand required = Objects.requireNonNull(command, "command must not be null");
    TacticSuite suite = ownedSuite(required.ownerId(), required.suiteId());
  ...
  public void deleteExercise(DeleteTacticExerciseCommand command) {
    assertAvailable();
    DeleteTacticExerciseCommand required = Objects.requireNonNull(command, "command must not be null");
    TacticSuite suite = ownedSuite(required.ownerId(), required.suiteId());
  ...
  public TacticWorkspace startExercise(PlayerId ownerId, TacticSuiteId suiteId, TacticExerciseId exerciseId) {
    assertAvailable();
    TacticSuite suite = ownedSuite(ownerId, suiteId);
  ...
  public TacticMoveOutcome attemptMove(
    ...
    if (!attempt.accept(result)) {
      return new TacticMoveOutcome(attempt.workspace("That move does not solve the tactic. Try again."), false);
    }
  ...
  public TacticHint requestHint(PlayerId ownerId, TacticSuiteId suiteId, TacticExerciseId exerciseId) {
    assertAvailable();
  ...
  public TacticAuthoringMoveOutcome appendSolutionMove(AppendTacticSolutionMoveCommand command) {
    ...
    if (!result.accepted()) {
      return new TacticAuthoringMoveOutcome(
          workspace(suite, exercise, basePosition, false, "That move is not legal."), Optional.empty(), false);
    }
    AnalysisNode node =
  ...
  private TacticExercise exercise(TacticSuite suite, TacticExerciseId exerciseId) {
    return suite
        ...
        .orElseThrow(() -> new NoSuchElementException("Unknown tactic exercise " + exerciseId.value()));
  }

  private TacticSuiteSummary summary(TacticSuite suite) {
    return new TacticSuiteSummary(suite.id(), suite.title(), suite.description(), suite.exercises().size(), suite.updatedAt());
  }

  private TacticExerciseSummary summary(TacticExercise exercise) {
    return new TacticExerciseSummary(exercise.id(), exercise.title(), exercise.solverColor(), exercise.hasSolution());
  }
  ...
  private TacticWorkspace workspace(
    ...
    return new TacticWorkspace(
        suite.id(), suite.title(), exercise.id(), exercise.title(), exercise.solverColor(), position,
        exercise.hasSolution(), solved, 0, 0, 0, 0, status);
  }
  ...
  private static final class Attempt {
    ...
    TacticWorkspace workspace() {
      if (!exercise.hasSolution()) return workspace("This tactic needs a solution line before it can be trained.");
      return workspace(solved ? "Solved!" : "Find the best move for " + colorName(exercise.solverColor()) + ".");
    }
    ...
    TacticWorkspace workspace(String status) {
      return new TacticWorkspace(
          ...
          hintCount,
          accuracyPercentage(),
          solved ? status + " Accuracy: " + accuracyPercentage() + "% (" + correctMoves + "/" + attemptedMoves + ")" : status);
    }
