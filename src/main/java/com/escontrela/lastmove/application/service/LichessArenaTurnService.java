...
public final class LichessArenaTurnService {
  ...
  public void consume(String gameId, JsonNode event, LichessBotAccount account, String token, LichessBotClient client) {
    ...
    if ("gameFull".equals(type)) {
      ...
      Runtime old = runtimes.put(gameId, new Runtime(event.path("initialFen").asText("startpos"), color, playerName(white,"White"), playerName(black,"Black")));
  ...
  private ChessGame replay(String initialFen, String moves, Runtime runtime) {
    ChessGame game = "startpos".equals(initialFen) ? games.createInitial(new GamePlayer(runtime.whiteName,PieceColor.WHITE),new GamePlayer(runtime.blackName,PieceColor.BLACK),Optional.of(TimeControl.unlimited())) : games.createFrom(Fen.of(initialFen),new GamePlayer(runtime.whiteName,PieceColor.WHITE),new GamePlayer(runtime.blackName,PieceColor.BLACK),Optional.of(TimeControl.unlimited()));
  ...
  private static boolean matches(JsonNode player, LichessBotAccount account) { return account.id().equalsIgnoreCase(player.path("id").asText()) || account.username().equalsIgnoreCase(player.path("name").asText()); }
  private static String playerName(JsonNode player,String fallback){String name=player.path("name").asText();if(name.isBlank())name=player.path("username").asText();if(name.isBlank())name=player.path("id").asText();return name.isBlank()?fallback:name;}
  private static int plyCount(String moves){return moves==null||moves.isBlank()?0:moves.trim().split("\\s+").length;}
  private static final class Runtime { final String initialFen; final PieceColor color; final String whiteName,blackName; final AtomicBoolean thinking = new AtomicBoolean(); volatile String moves = ""; volatile JsonNode state; volatile ComputerMoveEngine engine; volatile GameId localGameId; Runtime(String initialFen, PieceColor color,String whiteName,String blackName) { this.initialFen=initialFen; this.color=color;this.whiteName=whiteName;this.blackName=blackName; } void close() { if(engine != null) { engine.cancelSearch(); engine.close(); } } }
}
