...
public class SqliteLichessArenaRepository implements LichessArenaRepository {
  ...
  public void saveGame(ArenaGame value) {
    if (!availability.isAvailable()) return;
    jdbc.update("INSERT INTO lichess_games(lichess_game_id,local_game_id,challenge_id,tournament_id,game_url,white_lichess_id,black_lichess_id,bot_color,remote_status,last_error,started_at,finished_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(lichess_game_id) DO UPDATE SET local_game_id=excluded.local_game_id,challenge_id=excluded.challenge_id,tournament_id=excluded.tournament_id,game_url=excluded.game_url,white_lichess_id=excluded.white_lichess_id,black_lichess_id=excluded.black_lichess_id,bot_color=excluded.bot_color,remote_status=excluded.remote_status,last_error=excluded.last_error,finished_at=excluded.finished_at,updated_at=excluded.updated_at",value.lichessGameId(),value.localGameId().map(g->g.value().toString()).orElse(null),value.challengeId().orElse(null),value.tournamentId().orElse(null),value.gameUrl().orElse(null),value.whiteLichessId().orElse(null),value.blackLichessId().orElse(null),value.botColor().map(Enum::name).orElse(null),value.status().name(),value.lastError().orElse(null),epoch(value.startedAt()),epoch(value.finishedAt()),epoch(value.updatedAt()));
  }
  ...
  public List<ArenaGame> listActiveGames() { return availability.isAvailable()?jdbc.queryForList("SELECT * FROM lichess_games WHERE remote_status IN ('STARTED','ACTIVE','FINISHED','STREAM_CLOSED','ERROR') ORDER BY updated_at DESC").stream().map(this::game).toList():List.of(); }
  public void saveTournament(ArenaTournament value) {
    if (!availability.isAvailable()) return;
    jdbc.update("INSERT INTO lichess_tournaments(lichess_tournament_id,name,remote_status,variant,rated,clock_limit_seconds,clock_increment_seconds,duration_minutes,player_count,minimum_rating,maximum_rating,bots_allowed,starts_at,finishes_at,seconds_to_start,tournament_url,registration_status,last_error,last_seen_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(lichess_tournament_id) DO UPDATE SET name=excluded.name,remote_status=excluded.remote_status,variant=excluded.variant,rated=excluded.rated,clock_limit_seconds=excluded.clock_limit_seconds,clock_increment_seconds=excluded.clock_increment_seconds,duration_minutes=excluded.duration_minutes,player_count=excluded.player_count,minimum_rating=excluded.minimum_rating,maximum_rating=excluded.maximum_rating,bots_allowed=excluded.bots_allowed,starts_at=excluded.starts_at,finishes_at=excluded.finishes_at,seconds_to_start=excluded.seconds_to_start,tournament_url=excluded.tournament_url,registration_status=excluded.registration_status,last_error=excluded.last_error,last_seen_at=excluded.last_seen_at,updated_at=excluded.updated_at", value.lichessTournamentId(),value.name(),value.remoteStatus().name(),value.variant(),value.rated()?1:0,value.clockLimitSeconds(),value.clockIncrementSeconds(),value.durationMinutes(),value.playerCount(),value.minimumRating().orElse(null),value.maximumRating().orElse(null),value.botsAllowed()?1:0,epoch(value.startsAt()),epoch(value.finishesAt()),value.secondsToStart().orElse(null),value.url().orElse(null),value.registrationStatus().name(),value.lastError().orElse(null),epoch(value.lastSeenAt()),epoch(value.updatedAt()));
  }
  public Optional<ArenaTournament> findTournament(String id) { return availability.isAvailable()?jdbc.queryForList("SELECT * FROM lichess_tournaments WHERE lichess_tournament_id=?",id).stream().findFirst().map(this::tournament):Optional.empty(); }
  public List<ArenaTournament> listTournaments() { return availability.isAvailable()?jdbc.queryForList("SELECT * FROM lichess_tournaments ORDER BY starts_at DESC, updated_at DESC").stream().map(this::tournament).toList():List.of(); }
  ...
  private ArenaGame game(Map<String,Object> r){ return new ArenaGame((String)r.get("lichess_game_id"),opt((String)r.get("local_game_id")).map(v->new GameId(UUID.fromString(v))),opt((String)r.get("challenge_id")),opt((String)r.get("tournament_id")),opt((String)r.get("game_url")),opt((String)r.get("white_lichess_id")),opt((String)r.get("black_lichess_id")),opt((String)r.get("bot_color")).map(PieceColor::valueOf),ArenaGameStatus.valueOf((String)r.get("remote_status")),opt((String)r.get("last_error")),Instant.ofEpochMilli(((Number)r.get("started_at")).longValue()),instant(r.get("finished_at")),Instant.ofEpochMilli(((Number)r.get("updated_at")).longValue())); }
  private ArenaTournament tournament(Map<String,Object> r){ return new ArenaTournament((String)r.get("lichess_tournament_id"),(String)r.get("name"),ArenaTournamentStatus.valueOf((String)r.get("remote_status")),(String)r.get("variant"),((Number)r.get("rated")).intValue()!=0,((Number)r.get("clock_limit_seconds")).intValue(),((Number)r.get("clock_increment_seconds")).intValue(),((Number)r.get("duration_minutes")).intValue(),((Number)r.get("player_count")).intValue(),number(r.get("minimum_rating")),number(r.get("maximum_rating")),((Number)r.get("bots_allowed")).intValue()!=0,instant(r.get("starts_at")),instant(r.get("finishes_at")),number(r.get("seconds_to_start")),opt((String)r.get("tournament_url")),ArenaTournamentRegistrationStatus.valueOf((String)r.get("registration_status")),opt((String)r.get("last_error")),Instant.ofEpochMilli(((Number)r.get("last_seen_at")).longValue()),Instant.ofEpochMilli(((Number)r.get("updated_at")).longValue())); }
