...
public class SqliteGameNotificationRepository implements GameNotificationRepository {
  ...
  public void notify(PlayerId owner, GameId game, String kind) {
    if (!availability.isAvailable()) return;
    if ("GAME_FINISHED".equals(kind)) {
      jdbc.update("DELETE FROM game_notifications WHERE owner_player_id=? AND game_id=? AND kind IN ('GAME_CREATED','OPPONENT_MOVED')", owner.value(), game.value().toString());
    } else if ("OPPONENT_MOVED".equals(kind)) {
      jdbc.update("DELETE FROM game_notifications WHERE owner_player_id=? AND game_id=? AND kind IN ('GAME_CREATED','OPPONENT_MOVED')", owner.value(), game.value().toString());
    }
    jdbc.update("INSERT INTO game_notifications(id,owner_player_id,game_id,kind,created_at) VALUES(?,?,?,?,?)", UUID.randomUUID().toString(),owner.value(),game.value().toString(),kind,Instant.now().toEpochMilli());
  }
