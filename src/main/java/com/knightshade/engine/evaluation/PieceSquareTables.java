package com.knightshade.engine.evaluation;

import com.escontrela.lastmove.domain.common.PieceType;

/**
 * Piece-square tables from White's perspective, stored in a1-first (rank-major) order.
 *
 * <p>Values are the "simplified evaluation function" tables (Chess Programming Wiki) mirrored so
 * index 0 is a1. Black uses the same table mirrored across the board via {@code index ^ 56}.
 */
public final class PieceSquareTables {

  private static final int[] PAWN = {
    0, 0, 0, 0, 0, 0, 0, 0,
    5, 10, 10, -20, -20, 10, 10, 5,
    5, -5, -10, 0, 0, -10, -5, 5,
    0, 0, 0, 20, 20, 0, 0, 0,
    5, 5, 10, 25, 25, 10, 5, 5,
    10, 10, 20, 30, 30, 20, 10, 10,
    50, 50, 50, 50, 50, 50, 50, 50,
    0, 0, 0, 0, 0, 0, 0, 0
  };

  private static final int[] KNIGHT = {
    -50, -40, -30, -30, -30, -30, -40, -50,
    -40, -20, 0, 5, 5, 0, -20, -40,
    -30, 5, 10, 15, 15, 10, 5, -30,
    -30, 0, 15, 20, 20, 15, 0, -30,
    -30, 5, 15, 20, 20, 15, 5, -30,
    -30, 0, 10, 15, 15, 10, 0, -30,
    -40, -20, 0, 0, 0, 0, -20, -40,
    -50, -40, -30, -30, -30, -30, -40, -50
  };

  private static final int[] BISHOP = {
    -20, -10, -10, -10, -10, -10, -10, -20,
    -10, 5, 0, 0, 0, 0, 5, -10,
    -10, 10, 10, 10, 10, 10, 10, -10,
    -10, 0, 10, 10, 10, 10, 0, -10,
    -10, 5, 5, 10, 10, 5, 5, -10,
    -10, 0, 5, 10, 10, 5, 0, -10,
    -10, 0, 0, 0, 0, 0, 0, -10,
    -20, -10, -10, -10, -10, -10, -10, -20
  };

  private static final int[] ROOK = {
    0, 0, 0, 5, 5, 0, 0, 0,
    -5, 0, 0, 0, 0, 0, 0, -5,
    -5, 0, 0, 0, 0, 0, 0, -5,
    -5, 0, 0, 0, 0, 0, 0, -5,
    -5, 0, 0, 0, 0, 0, 0, -5,
    -5, 0, 0, 0, 0, 0, 0, -5,
    5, 10, 10, 10, 10, 10, 10, 5,
    0, 0, 0, 0, 0, 0, 0, 0
  };

  private static final int[] QUEEN = {
    -20, -10, -10, -5, -5, -10, -10, -20,
    -10, 0, 5, 0, 0, 0, 0, -10,
    -10, 5, 5, 5, 5, 5, 0, -10,
    0, 0, 5, 5, 5, 5, 0, -5,
    -5, 0, 5, 5, 5, 5, 0, -5,
    -10, 0, 5, 5, 5, 5, 0, -10,
    -10, 0, 0, 0, 0, 0, 0, -10,
    -20, -10, -10, -5, -5, -10, -10, -20
  };

  private static final int[] KING = {
    20, 30, 10, 0, 0, 10, 30, 20,
    20, 20, 0, 0, 0, 0, 20, 20,
    -10, -20, -20, -20, -20, -20, -20, -10,
    -20, -30, -30, -40, -40, -30, -30, -20,
    -30, -40, -40, -50, -50, -40, -40, -30,
    -30, -40, -40, -50, -50, -40, -40, -30,
    -30, -40, -40, -50, -50, -40, -40, -30,
    -30, -40, -40, -50, -50, -40, -40, -30
  };

  /** Smoothly trades shelter for central king activity as non-pawn material disappears. */
  public static int kingValue(int index, boolean white, int phase) {
    int file = index & 7;
    int rank = index >>> 3;
    int centerDistance = Math.max(3 - file, file - 4) + Math.max(3 - rank, rank - 4);
    int endgame = 40 - 15 * centerDistance;
    return (value(PieceType.KING, index, white) * phase
        + endgame * (GamePhase.MAX - phase)) / GamePhase.MAX;
  }

  private PieceSquareTables() {}

  /** Returns the positional bonus for a piece on the given square index. */
  public static int value(PieceType type, int index, boolean white) {
    int[] table =
        switch (type) {
          case PAWN -> PAWN;
          case KNIGHT -> KNIGHT;
          case BISHOP -> BISHOP;
          case ROOK -> ROOK;
          case QUEEN -> QUEEN;
          case KING -> KING;
        };
    int square = white ? index : (index ^ 56);
    return table[square];
  }
}
