...
class LichessArenaServiceTest {
  ...
  @Test void resilienceSupervisorRecoversAPlayedGameFromAnErroredCycleAndChallengesNextBot() throws Exception {
    ...
    repo.cycle = new BotChallengeCycle(BotChallengeCycleStatus.ERROR,
        ...
        Optional.empty(), Optional.of("Missing challenge id"), now);
    repo.saveGame(new ArenaGame("played", Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty(),
        Optional.of("knight-shade-bot"), Optional.of("one"), Optional.empty(), ArenaGameStatus.FINISHED,
        Optional.empty(), now, Optional.of(now), now));
    client.bots = List.of(new LichessBotCandidate("one", "one", true, true, Optional.of(1400), Optional.empty(), now),
        new LichessBotCandidate("two", "two", true, true, Optional.of(1500), Optional.empty(), now));
    BotChallengeCycleService cycle = new BotChallengeCycleService(repo, settings, client, event -> {});
    LichessArenaService service = new LichessArenaService(repo, settings, client, event -> {});
    var field = LichessArenaService.class.getDeclaredField("botCycle"); field.setAccessible(true); field.set(service, cycle);
    service.connect();

    service.superviseResilience();

    assertEquals(1, repo.cycle.completedGames());
    assertEquals("two", repo.cycle.currentBotId().orElseThrow());
    assertEquals(BotChallengeCycleStatus.WAITING_FOR_GAME, repo.cycle.status());
    service.closeOnApplicationExit();
  }
