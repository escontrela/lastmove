...
class LichessArenaServiceTest {
  ...
  private static final class FakeClient implements LichessBotClient { String accepted; Consumer<JsonNode> account; Consumer<Throwable> accountClosed; int accountSubscriptions,tournamentRequests; final Map<String,Integer> gameSubscriptions=new HashMap<>(); final Map<String,Consumer<Throwable>> gameClosed=new HashMap<>(); List<LichessTournamentSnapshot> tournaments=List.of(); RuntimeException tournamentFailure; public StreamHandle streamEvents(String t,Consumer<JsonNode> c,Consumer<Throwable> x){account=c;accountClosed=x;accountSubscriptions++;return ()->{};} public StreamHandle streamGame(String t,String id,Consumer<JsonNode> c,Consumer<Throwable> x){gameSubscriptions.merge(id,1,Integer::sum);gameClosed.put(id,x);return ()->{};} public JsonNode currentGames(String t){return new ObjectMapper().createObjectNode().putArray("nowPlaying");} public List<LichessTournamentSnapshot> currentTournaments(String t){tournamentRequests++;if(tournamentFailure!=null)throw tournamentFailure;return tournaments;} public void acceptChallenge(String t,String id){accepted=id;} public void declineChallenge(String t,String id,String r){} public void sendMove(String t,String g,String u){} public void resign(String t,String g){} public void offerDraw(String t,String g){} }
}
